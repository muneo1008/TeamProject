...
import {likedSnaps, mySnaps, OtherLikedSnapInfo, OtherSnapInfo} from "../api.jsx";
...
const UserSnap = (props) => {
    ...
    const getMySnaps = async (memberId) => {
        try {
            const result = await OtherSnapInfo(memberId);
    ...
    const getLikedSnaps = async (memberId) => {
        try {
            const result = await OtherLikedSnapInfo(memberId);
